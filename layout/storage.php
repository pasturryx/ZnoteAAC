...
// Define your storage arrays
$AccountStorageKeys = array();

$GlobalStorageKeys = array(
	FuryGates => 100,

	$TheirMastersVoice = array(
		CurrentServantWave => 984,
		ServantsKilled => 985
	),

	InServiceOfYalahar = array(
		LastFight => 982,
...
PlayerStorageKeys = array(
	SweetyCyclops = array(
		AmuletTimer => 48,
		AmuletStatus => 49
	),

	ExplorerSociety = array(
		QuestLine => 90,
		bansheeDoor => 91,
		bonelordsDoor => 92,
		calassaDoor => 93,
	...
	WhiteRavenMonasteryQuest = array(
		QuestLog => 50200,
		Passage => 50201,
		Diary => 50202
	),

	FathersBurdenQuest = array(
		QuestLog => 50203,
		Progress => 50204,
		Status => 50205,
		Sinew => 50206,
...
// Function to extract storage IDs
function extractStorageIDs($array, &$storageList) {
    foreach ($array as $key => $value) {
        if (is_int($value)) {
            $storageList[) = $value;
        } elseif (is_array($value)) {
            extractStorageIDs($value, $storageList);
        }
    }
}
...
function extractStorageIDs($array, &$storageList) {
    foreach ($array as $key => $value) {
        if (is_int($value) && stripos($key, 'reward') === false) {
            $storageList[) = $value;
        } elseif (is_array($value)) {
            extractStorageIDs($value, $storageList);
        }
...
// Extract storage IDs from all arrays
$storages = array();
extractStorageIDs($AccountStorageKeys, $storages);
extractStorageIDs($GlobalStorageKeys, $storages);
extractStorageIDs($PlayerStorageKeys, $storages);

// Function to print all storages (for debugging or saving to file)
function printStorages($storages) {
    foreach ($storages as $storage) {
        echo $storage . "\n";
    }
}

// Print all storage IDs (you can redirect this to a file if needed)
printStorages($storages);

// Example function that sets storage values on player login (assuming a player object with a setStorageValue method)
// This is a placeholder and should be replaced with actual game logic
function setStorageOnLogin($player, $storages) {
    foreach ($storages as $storage) {
        $player->setStorageValue($storage, 1);
    }
}

// Placeholder player class for demonstration purposes
class Player {
    public function setStorageValue($key, $value) {
        // Implement the logic to set the storage value for the player
        echo "Setting storage $key to $value\n";
    }
}

// Example usage with a dummy player object
$player = new Player();
setStorageOnLogin($player, $storages);
