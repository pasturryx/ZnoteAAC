...
function user_account_add_premium_ends_at($accid, $days) {
    ...
    if (config('ServerEngine') !== 'OTHIRE') {
        ...
        if ($hasPremiumEndsAt !== false) {
            mysql_update("UPDATE `accounts` SET `premium_ends_at` = GREATEST(`premium_ends_at`, UNIX_TIMESTAMP()) + ({$days} * 86400) WHERE `id`='{$accid}'");
        } elseif ($hasPremiumDays !== false) {
            mysql_update("UPDATE `accounts` SET `premiumdays` = `premiumdays` + {$days} WHERE `id`='{$accid}'");
        }
    } else {
