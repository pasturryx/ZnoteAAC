...
if (user_logged_in() === true) {
    ...
    if ($config['ServerEngine'] !== 'OTHIRE') {
        if ($config['ServerEngine'] == 'TFS_10') {
            $hasPremiumEndsAt = mysql_select_single("SHOW COLUMNS from `accounts` WHERE `Field` = 'premium_ends_at'");
            $hasPremiumDays = mysql_select_single("SHOW COLUMNS from `accounts` WHERE `Field` = 'premiumdays'");
            
            if ($hasPremiumEndsAt !== false) {
                $user_data = user_data($session_user_id, 'id', 'name', 'password', 'email', 'premium_ends_at');
                $user_data['premium_days_remaining'] = ($user_data['premium_ends_at'] - time() > 0) ? floor(($user_data['premium_ends_at'] - time()) / 86400) : 0;
            } elseif ($hasPremiumDays !== false) {
                $user_data = user_data($session_user_id, 'id', 'name', 'password', 'email', 'premiumdays');
                $user_data['premium_days_remaining'] = $user_data['premiumdays'];
            } else {
                $user_data = user_data($session_user_id, 'id', 'name', 'password', 'email');
                $user_data['premium_days_remaining'] = 0;
            }
        } else {
            $user_data = user_data($session_user_id, 'id', 'name', 'password', 'email');
            $user_data['premium_days_remaining'] = 0;
        ...
        $user_data['premium_days_remaining'] = ($user_data['premend'] - time() > 0) ? floor(($user_data['premend'] - time()) / 86400) : 0;
    }
