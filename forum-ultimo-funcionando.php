...
if ($admin) {
    $yourChars = mysql_select_multi("SELECT `id`, `name`, `group_id` FROM `players` WHERE `level`>='1' AND `account_id`='". $user_data['id'] ."';");
} else {
    $yourChars = mysql_select_multi("SELECT `id`, `name`, `group_id` FROM `players` WHERE `level`>='". $config['forum']['level'] ."' AND `account_id`='". $user_data['id'] ."';");
}
if (!$yourChars) {
    $yourChars = array();
}
...
$leader = false; // Initialize $leader variable outside of the if block
$charData = array(); // Initialize $charData array outside of the if block
...
if ($admin && !empty($_POST)) {
    ...
    foreach ($charData as $char) {
        if ($char['guild'] > 0 && $char['guild_rank'] == 3) {
            $leader = true;
        }
    }
}
...
if (($admin && !empty($_POST)) || ($leader && !empty($_POST))) {
    $admin_thread_delete = getValue($_POST['admin_thread_delete'] ?? false);
    $admin_thread_close = getValue($_POST['admin_thread_close'] ?? false);
    $admin_thread_open = getValue($_POST['admin_thread_open'] ?? false);
    $admin_thread_sticky = getValue($_POST['admin_thread_sticky'] ?? false);
    $admin_thread_unstick = getValue($_POST['admin_thread_unstick'] ?? false);
    $admin_thread_id = getValue($_POST['admin_thread_id'] ?? false);
    ...
    if ($admin_thread_delete !== false) {
        ...
        if (!$admin) {
            ...
            foreach($charData as $char) {
                if ($char['guild'] == $forum['guild_id'] && $char['guild_rank'] == 3) {
                    $access = true;
                }
            }
        } else {
        ...
        if ($access) {
            ...
            echo '<p><b><font color="red">Permission denied.</font></b></p>';
        }
    }
    ...
    if ($admin_thread_sticky !== false) {
        ...
        if ($access) {
            ...
            echo '<h1>Thread has been stickied.</h1>';
        } else {
            echo '<p><b><font color="red">Permission denied.</font></b></p>';
        }
    }
...
// ADMIN FUNCT
if ($admin && !empty($_POST)) {
    $admin_post_id = getValue($_POST['admin_post_id'] ?? false);
    $admin_post_delete = getValue($_POST['admin_post_delete'] ?? false);

    $admin_category_delete = getValue($_POST['admin_category_delete'] ?? false);
    $admin_category_edit = getValue($_POST['admin_category_edit'] ?? false);
    $admin_category_id = getValue($_POST['admin_category_id'] ?? false);

    $admin_update_category = getValue($_POST['admin_update_category'] ?? false);
    $admin_category_name = getValue($_POST['admin_category_name'] ?? false);

    $admin_category_access = getValue($_POST['admin_category_access'] ?? false);
    $admin_category_closed = getValue($_POST['admin_category_closed'] ?? false);
    $admin_category_hidden = getValue($_POST['admin_category_hidden'] ?? false);
    $admin_category_guild_id = getValue($_POST['admin_category_guild_id'] ?? false);
    ...
    $admin_board_create_name = getValue($_POST['admin_board_create_name'] ?? false);
    $admin_board_create_access = getValue($_POST['admin_board_create_access'] ?? false);
    $admin_board_create_closed = getValue($_POST['admin_board_create_closed'] ?? false);
    $admin_board_create_hidden = getValue($_POST['admin_board_create_hidden'] ?? false);
    $admin_board_create_guild_id = getValue($_POST['admin_board_create_guild_id'] ?? false);
